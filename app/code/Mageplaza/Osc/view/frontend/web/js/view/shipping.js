        ...
        return Component.extend({
            ...
            initialize: function () {
                ...
                if (window.checkoutConfig.hasOwnProperty('amazonLogin')) {
                 ...
                 registry.async('checkout.steps.shipping-step.billingAddress')(function (billingAddress) {
                        shippingService.setShippingRates(window.checkoutConfig.shippingMethods);
                 });
            ...
            initObservable: function () {
                ...
                shippingService.getShippingRates().subscribe(function (methods) {
                    var availableRate;
                    var test;
                    if(methods.length >0){
                    if (methods.length == 1) {
                        availableRate = methods[0];
                        window.checkoutConfig.selectedShippingMethod = availableRate;
                    }
                    $.each(methods, function (index, method) {
                       if(method.method_code =="freeshipping"){
                                availableRate = methods[index];
                            }
                    });
             ...
             isBestWay: function(test){
                if(test.method_code =="freeshipping"){
                    return true;
                }
                else{
                    return false;
            ...
            saveShippingAddress: function () {
                ...
                if (customer.isLoggedIn()|| oscData.getData('form_register_chechbox') ==true) {
